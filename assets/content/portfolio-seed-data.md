# Portfolio Seed Data — for Claude Code

Source: Novusfy case studies (5 of ~10 total; more may follow in a separate file).
Mapped to the `Portfolio` collection schema (see CLAUDE.md).

## Instructions for Claude Code

1. Do **not** hand-enter these through `/admin` — write a one-off seed script
   (e.g. `src/seed/portfolio.ts`, run via `tsx` or a temporary route/CLI
   command using the Payload Local API) that creates these 5 documents in the
   `Portfolio` collection, then delete/ignore the script afterward. This is
   faster and less error-prone than manual entry for structured data like this.
2. **Images:** every entry below uses the same placeholder. Before running
   the seed, check if a placeholder image already exists in `Media`; if not,
   create/upload one generic placeholder (e.g. a plain gray rectangle or a
   "Case study image coming soon" graphic — doesn't matter, it's temporary)
   and use its ID for every `coverImage` and `gallery` field across all 5
   entries. Owner will replace them one by one in `/admin` afterward.
3. `content` (richText) — convert the Markdown sections under each case
   below into Lexical richText nodes (headings, paragraphs, bullet lists,
   blockquote for "Key Impact"), preserving structure. Bold text in the
   source should become bold inline formatting, not literal `**`.
4. Leave `completedDate` blank/null for all 5 (no data available), and
   `externalLink` blank/null except for **DDTAJ**, which has a real link
   (`https://ddtaj.com`).
5. Set `featured: true` on **Erbil Hills**, **GAV TV**, and **DDTAJ** (the
   three strongest, most complete stories); leave `featured: false` on Korek
   Telecom and Aral Hope. This is a suggestion — owner can flip these in
   `/admin` at any time.
6. All 5 entries are final and complete — no pending fields remain.
7. All 5 entries: `_status: 'draft'` on creation (not published), so the
   owner can review each on-site before making it public. Owner will flip to
...
### summary (card teaser, 1–2 sentences)
...
### results array
_(none — no quantifiable stats in source; full story lives in content above)_

---

## 2. GAV TV — Digital Transformation for News Media
...
- **featured:** true
- **location note:** Iraq (Kurdistan Region) — not a schema field, mention in content if useful
...
### results array (quantifiable — use these on the case study card/highlights)
| label | value |
|---|---|
| Page load time reduction | 87% (8.2s → 1.1s) |
| Uptime during traffic spikes | 99.8% |
| Concurrent user capacity | 5x increase |
| Mobile PageSpeed score | 32 → 94 |
| Organic search traffic (6 months) | +340% |
| Bounce rate | 68% → 37% (−45%) |

_(Additional stats from source not included above to keep the card skimmable — first-page rankings for 127 keywords, 23 featured snippets, 3.2x session duration, 89% pages-per-session improvement, mobile traffic 42%→67% — all still available in the full `content` body.)_
...
## 4. DDTAJ — Building an Agency From the Ground Up

- **slug:** `ddtaj`
- **clientName:** DDTAJ
- **industry:** Marketing & Digital Consultancy (currently active as Influencer Marketing & Conference/Event Organizing)
...
- Operates under its own brand at [ddtaj.com](https://ddtaj.com), based in Duhok
...
### results array
_(none yet — no independent build-side growth metrics provided. DDTAJ's own client-facing marketing stats, e.g. ROI/uptime figures from their corporate profile, are their claims to their clients, not a measure of Novusfy's build work, and are deliberately excluded here to avoid misattribution.)_

---

## 5. Aral Hope Medical Complex (incl. Lunava Pharmacy)
